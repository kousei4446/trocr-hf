...
def _load_uttlist(path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        for idx, line in enumerate(f, 1):
            ...
            parts = line.split()
            if len(parts) != 1:
                print(f"Warning: skip line {idx} in {os.path.basename(path)} (expected 1 column, got {len(parts)})")
                continue
            ids.append(parts[0])
    return ids
...
def prepare_iam_data(form_path, xmls_path, splits_path, output_path, pad_size=16, scale=1.0):
    ...
    train_set = _load_uttlist(os.path.join(splits_path, 'train.uttlist'))
    val_set = _load_uttlist(os.path.join(splits_path, 'validation.uttlist'))
    test_set = _load_uttlist(os.path.join(splits_path, 'test.uttlist'))
    ...
    for xml_file in tqdm(xml_files):
        ...
        for line in lines:
            ...
            line_file = os.path.join(output_path, subset, "images", line_id + ".png")
    ...
    for subset in gt_lines.keys():
        with open(os.path.join(output_path, subset, "labels.txt"), "w", encoding="utf-8") as f:
